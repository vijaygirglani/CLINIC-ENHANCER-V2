                    ...
                    <div className="relative">
                      <div className="flex gap-2">
                        <input
                          ...
                          onBlur={() => setTimeout(() => setShowNameDropdown(false), 180)}
                          className="w-full px-4 py-3 rounded-xl bg-white border border-slate-200 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-slate-800"
                      ...
                      <AnimatePresence>
                        {showNameDropdown && nameSuggestions.length > 0 && (
                          <motion.div
                            ...
                            className="absolute left-0 right-0 top-full mt-1.5 z-50 bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden"
                          ...
                          >
                            ...
                            {nameSuggestions.map((s, i) => (
                              <button
                                ...
                                className="w-full px-4 py-3 hover:bg-primary/5 transition-colors text-left border-b border-slate-50 last:border-0 group"
                              >
                                <div className="flex items-start gap-3">
                                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0 mt-0.5 group-hover:bg-primary/20 transition-colors">
                                    <User className="w-3.5 h-3.5" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    {/* Row 1: Name + visit badge */}
                                    ...
                                    {/* Row 2: Case/mobile + age + address */}
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                      <span className="text-xs font-mono text-slate-500">{s.mobile}</span>
                                      {s.age > 0 && (
                                        <span className="text-xs text-slate-400">{s.age}{s.ageMonths ? `y ${s.ageMonths}m` : "y"}</span>
                                      )}
                                      {s.address && (
                                        <span className="text-xs text-slate-400 truncate max-w-[120px]">· {s.address}</span>
                                      )}
                                    </div>
                                    {/* Row 3: Last visit preview */}
                                    {s.recentVisits[0] && (
                                      <div className="mt-1 text-[10px] text-slate-400 flex items-center gap-1.5">
                                        ...
                                        {s.recentVisits[0].complaint && (
                                          <span className="truncate max-w-[180px]">· {s.recentVisits[0].complaint.slice(0, 35)}</span>
                                        )}
                                      </div>
                                    )}
                                  </div>
                                  <span className="text-[10px] text-primary font-semibold shrink-0 opacity-0 group-hover:opacity-100 transition-opacity mt-1">
                                    Fill →
                                  </span>
                                </div>
                              </button>

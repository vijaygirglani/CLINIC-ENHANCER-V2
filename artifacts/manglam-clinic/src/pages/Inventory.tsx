...
export default function Inventory() {
  ...
  // ── Medicines tab filters ──
  const [medSearch, setMedSearch] = useState("");
  const [medPharmacyFilter, setMedPharmacyFilter] = useState("");

  // Pharmacy names actually attached to medicines, plus any from the Pharmacies
  // tab, so the dropdown covers suppliers you've registered but not yet bought from.
  const medicinePharmacyOptions = Array.from(new Set([
    ...medicines.map(m => (m.supplierName || "").trim()).filter(Boolean),
    ...pharmacies.map(p => p.name.trim()).filter(Boolean),
  ])).sort((a, b) => a.localeCompare(b));

  const filteredMedicines = medicines.filter(m => {
    const supplier = (m.supplierName || "").trim();
    if (medPharmacyFilter === "__none__" && supplier) return false;
    if (medPharmacyFilter && medPharmacyFilter !== "__none__" && supplier.toLowerCase() !== medPharmacyFilter.toLowerCase()) return false;
    if (medSearch.trim() && !m.name.toLowerCase().includes(medSearch.trim().toLowerCase())) return false;
    return true;
  });
  const filteredStockQty = filteredMedicines.reduce((s, m) => s + (m.currentStock || 0), 0);
  const filteredStockValue = filteredMedicines.reduce((s, m) => s + (m.currentStock || 0) * m.landingCost, 0);
          ...
          <div className="medical-card overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 space-y-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <button onClick={openNewMedicine} className="px-4 py-2 rounded-xl font-semibold bg-primary text-white shadow-lg shadow-primary/25 hover:shadow-xl transition-all flex items-center gap-1.5 text-sm shrink-0">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  ...
                  <input value={medSearch} onChange={e => setMedSearch(e.target.value)} placeholder="Search medicine name…"
                    className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none text-sm bg-white" />
                </div>
                <select value={medPharmacyFilter} onChange={e => setMedPharmacyFilter(e.target.value)}
                  className="px-4 py-2.5 rounded-xl border border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none text-sm bg-white sm:w-56">
                  <option value="">All pharmacies</option>
                  {medicinePharmacyOptions.map(name => <option key={name} value={name}>{name}</option>)}
                  <option value="__none__">— No pharmacy set —</option>
                </select>
                {(medSearch || medPharmacyFilter) && (
                  <button onClick={() => { setMedSearch(""); setMedPharmacyFilter(""); }}
                    className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium flex items-center justify-center gap-1.5">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredMedicines.length > 0 ? filteredMedicines.map(m => (
                    ...
                    <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      {medicines.length === 0
                        ? "No medicines yet. Add a name, pharmacy, cost and quantity to get started."
